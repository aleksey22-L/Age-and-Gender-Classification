...
#             By Aleksey L, 2021-03-11              #
#       Gender: SVM Classifier                      #
#       Age: caffemodel                             #
#####################################################

# Import Python Packages related to classify Gender & Age
import cv2
...
class Gender():
    ...
    def __init__(self):
        self.samples_path = "models/dataset/colored/jpg4/"
        self.samples_cnt = 0
    ...
    def create_svm_classifier(self):
        ...
        while iterates > 0:
            ...
            svm_classifier.fit(traineddata, trainLabels)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1 or sys.argv[2].split('.')[1] != "jpg":
        parser.print_help()
    else:
        ...
        frame = cv2.imread(str(sys.argv[2]))

        result_face, faceBoxes = face.detect_face(frame)
        padding = 20

        if not faceBoxes:
            print("No Face Detecting")
        else:
            for faceBox in faceBoxes:
                ...
                cv2.putText(result_face, f'{M_or_F}, {pre_age}', (faceBox[0], faceBox[1]-10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2, cv2.LINE_AA)
                cv2.imshow("Detecting age and gender", result_face)
